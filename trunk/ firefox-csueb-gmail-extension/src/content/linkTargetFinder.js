				...
				if (content.document.getElementById("canvas_frame")){
					...
					if (span_zF) {
						for (i=0; i<span_zF.length; i++){
							span_zF[i].addEventListener("mouseover", function() {popup_timer = setTimeout(linkTargetFinder.fireTool,1100,this);}, false);
							span_zF[i].addEventListener("mouseout", function() {clearTimeout(popup_timer);}, false);
						}
					}
					if (span_yP) {
						for (i=0; i<span_yP.length; i++){
							span_yP[i].addEventListener("mouseover", function() {popup_timer = setTimeout(linkTargetFinder.fireTool,1100,this);}, false);
